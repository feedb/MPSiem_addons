...
if __name__ == "__main__":
    ...
    settings['clear_method_url'] = '/api/events/v1/table_lists/tor_servers/clear'
    # урл метода импорта данных
    settings['import_method_url'] = '/api/events/v1/table_lists/tor_servers/import'
    # урл формы аутентификации
    settings['signin_form_url'] = '/account/login?returnUrl=/#/authorization/landing'
    # урл файла tor ips
    settings['tor ips_url'] = 'https://www.dan.me.uk/torlist/'
    ...
    # аутентификация прошла
        ...
        # загружаем свежий tor ips
        resp = requests.get(settings['tor ips_url'], stream=True)

        # если всё хорошо
        if resp.status_code == 200:
            cur_date = datetime.datetime.now().strftime("%d.%m.%Y %H:%M:%S")
            file_name = 'torlist_' + cur_date + '.csv'
        ...
        # ошибка аутентификации
            ...
            print("Не удалось импортировать новый tor ips.")
